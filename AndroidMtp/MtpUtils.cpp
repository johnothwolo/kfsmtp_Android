...
int copyRecursive(const char *fromPath, const char *toPath) {
    ...
    if (!dir) {
        std::cerr << "opendir " << fromPath << " failed";
        return -1;
    }
...
int copyFile(const char *fromPath, const char *toPath) {
    ...
    if (fromFd == -1) {
        std::cerr << "Failed to open copy from " << fromPath;
        return -1;
    }
    android::base::unique_fd toFd(open(toPath, O_CREAT | O_WRONLY, FILE_PERM));
    if (toFd == -1) {
        std::cerr << "Failed to open copy to " << toPath;
        return -1;
    }
    off_t offset = 0;
...
//        ret = sendfile(toFd, fromFd, &offset, transfer_length);
        if (ret != transfer_length) {
            ret = -1;
            std::cerr << "Copying failed!";
            break;
        }
    }
    auto end = std::chrono::steady_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << "Copied a file with MTP. Time: " << diff.count() << " s, Size: " << length <<
...
void deleteRecursive(const char* path) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (success == -1)
            std::cerr << "Deleting path " << childPath << " failed";
    }
    closedir(dir);
}
...
bool deletePath(const char* path) {
    ...
    if (stat(path, &statbuf) == 0) {
        ...
        std::cerr << "deletePath stat failed for " << path;
        return false;
    }
    if (success == -1)
        std::cerr << "Deleting path " << path << " failed";
    access_ok(path);
    return success == 0;
}
